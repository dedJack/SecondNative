...
const TouchableWithoutFeedbackDemo: React.FC = () => {
  ...
  return (
    <View style={styles.container}>
      ...
      <Pressable style={({pressed}) => [styles.btn,{ backgroundColor:pressed ? "white" : 'lightgrey' }]}> <Text style={styles.btnText}>Pressable</Text></Pressable>

      {/* another way to use pressable on texts because pressable doesnt render text itself*/}
      <Pressable style={styles.btn}>
        {({pressed})=>(
            <Text style={[styles.btnText,{color:pressed ? "red" : "white"}]}>Pressable 2</Text>
        )}
      </Pressable>
    </View>
  );
